...
It was just a reader writer problem which was provided in the sample files for projects. In that changes were made to call our own semaphore functions which were implemented in sem.h and run() was used in main to start execution and help in context switching.
...
void writer_exit(int ID)
{
	...
	wc--;
	if (rwc > 0) 
	{ // first, i let all the readers go.
   		V(&r_sem);
	} 
	else if (wwc > 0) 
	{ // then i let another writer go
		V(&w_sem);
	} 
	else 
	{ // lastly, i give up the mutex
		V(&mutex);
	}
}
